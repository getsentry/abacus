...
Provision a new OpenRouter API key for a team member directly from the CLI:

```bash
# Single workspace — workspace is selected automatically
pnpm cli openrouter:create-key jane@example.com "Claude Code"

# Multiple workspaces — --workspace is required
pnpm cli openrouter:create-key jane@example.com "Claude Code" --workspace "Coding Agents"
pnpm cli openrouter:create-key jane@example.com "Tooling" --workspace "Tools"
```

| Argument / Option | Description |
|-------------------|-------------|
| `<email>` | Team member's email address |
| `<name>` | Display name for the API key on OpenRouter |
| `--workspace <name>` | Workspace to provision the key in. Optional when zero or one workspaces are enabled (defaults to the enabled or account-default workspace); required when two or more are enabled. |

<Aside type="note">
  `--workspace` must match the name of an admin-enabled workspace exactly (case-sensitive). Admins manage the enabled set on the `/api-keys` page — see [OpenRouter workspaces](/abacus/providers/openrouter/#workspaces).
</Aside>
