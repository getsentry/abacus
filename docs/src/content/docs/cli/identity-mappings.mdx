...
| `--workspace <name>` | Workspace to provision the key in. Optional when exactly one workspace is configured; required when two or more are configured. |

<Aside type="note">
  `--workspace` must match a workspace name defined in `OPENROUTER_MANAGEMENT_KEYS` exactly (case-sensitive). Run `pnpm cli openrouter:status` to see configured workspace names.
</Aside>
