...
| Variable | Description |
|----------|-------------|
| `OPENROUTER_MANAGEMENT_KEY` | OpenRouter Management API key (starts with `sk-or-v1-`). Account-global — covers all workspaces. Powers key provisioning and usage sync. |

Which OpenRouter workspaces users may create keys in is managed by admins in the app, not via environment variables — see [OpenRouter workspaces](/abacus/providers/openrouter/#workspaces).
...
| `FLAGS` | Vercel Flags SDK keys. Provisioned automatically when you create your first flag in the Vercel dashboard (Project → Flags). Backs the `api-keys-page` flag that gates the `/api-keys` page. Without it the page is visible in development and hidden in production/preview. |
| `FLAGS_SECRET` | Enables flag overrides via the Vercel Toolbar's Flags Explorer. 32 random bytes, base64-encoded. |
