...
| `OAUTH_PROXY_SECRET` | Shared secret for the OAuth proxy that enables Google login on localhost and Vercel preview deployments. Must be the **same value** in production, preview, and local environments (unlike `BETTER_AUTH_SECRET`, which should differ per environment). Generate with: `openssl rand -hex 32` |
