...
<Aside type="note">
  To activate OpenRouter syncing, set `OPENROUTER_MANAGEMENT_KEY` in your Vercel project environment variables. This is a manual post-merge step — the cron will skip gracefully if the key is not set. The management key is account-global; per-workspace key creation is configured by admins in the app (see [OpenRouter workspaces](/abacus/providers/openrouter/#workspaces)).
</Aside>
