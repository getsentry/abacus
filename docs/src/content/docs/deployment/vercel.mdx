...
| `/api/cron/sync-openrouter` | Daily (00:30 UTC) | Sync recent OpenRouter usage (the activity API only exposes completed UTC days) |
...
<Aside type="note">
  To activate OpenRouter syncing, set `OPENROUTER_MANAGEMENT_KEY` (single workspace) or `OPENROUTER_MANAGEMENT_KEYS` (multiple workspaces) in your Vercel project environment variables. This is a manual post-merge step — the cron will skip gracefully if neither key is set.

  When setting `OPENROUTER_MANAGEMENT_KEYS` in the Vercel environment variable UI, paste the raw JSON **without** surrounding shell quotes:

  ```
  {"Coding Agents":"sk-or-v1-...","Tools":"sk-or-v1-..."}
  ```

  In `.env` files (local development), wrap the JSON in single quotes to prevent shell interpretation:
